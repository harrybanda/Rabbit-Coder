...
(async function () {
  const [
    ...
    Scene.root.findFirst("spikes"),
    Scene.root.findByPath("**/switches/*"),
    Scene.root.findByPath("**/buttons/*"),
    Scene.root.findFirst("carrot"),
    Scene.root.findFirst("water_emitter"),
    Scene.root.findFirst("instructions_view"),
    Scene.root.findFirst("congrats_view"),
    Scene.root.findFirst("UI"),
    Textures.findUsingPattern("*"),
    Materials.findUsingPattern("*"),
    Scene.root.findAll("knob"),
    // Sounds
    ...
    Audio.getAudioPlaybackController("switch"),
    Audio.getAudioPlaybackController("spikes_off"),
    Audio.getAudioPlaybackController("remove"),
    Audio.getAudioPlaybackController("pop"),
  ]);
      ...
      switch (i) {
        ...
        case 3:
          if (!loopAdded && activateLoopFunctionality === true) {
            ...
            loopIterations = 2;
            addCommand("loop_2");
            setTexture("loop_2_block", "loop_2_block_mat");
            setTexture("loop_off", "btn_loop_mat");
          }
          break;
        case 4:
          if (!endLoopAdded && activateLoopFunctionality === true) {
            endLoopAdded = true;
            addCommand("end_loop");
            setTexture("end_loop_off", "btn_end_loop_mat");
          }
          break;
        case 5:
          setLoopIterations(2);
  ...
  TouchGestures.onTap(blocks[10]).subscribe(function () {
    ...
    if (blocksUsed !== 0 && currentState === states.start) {
      ...
      if (popped.command.search("loop_") !== -1) {
        loopAdded = false;
        setTexture("loop", "btn_loop_mat");
      } else if (popped.command === "end_loop") {
        endLoopAdded = false;
        setTexture("end_loop", "btn_end_loop_mat");
      }
    }
  });
  ...
  Reactive.monitorMany({
    ...
    if (
      ...
      if (blocksUsed > maxBlocks) {
        Diagnostics.log(
          "You can also solve this with " + maxBlocks + " blocks."
        );
      }
    }
    ...
    if ("obstacle" in levels[currentLevel]) {
      ...
      for (let i = 0; i < switchCoords.length; i++) {
        ...
        if (
          ...
          let s = switches[i];
          knobs[i].transform.z = 0;
          player.transform.y = playerInitY + 0.015;
          if (knobs[i].transform.z.pinLastValue() !== 0) {
  ...
  function initLevel() {
    ...
    for (let i = 0; i < pathCoordinates.length; i++) {
      ...
      let x = path[0];
      let z = path[1];
      let platform = platforms[platformsUsed++];
    ...
    if (currentLevel > 4) {
      ...
      setTexture("loop", "btn_loop_mat");
      setTexture("end_loop", "btn_end_loop_mat");
      setTexture("loop_2", "btn_2_mat");
      setTexture("loop_3", "btn_3_mat");
      setTexture("loop_4", "btn_4_mat");
    }
    ...
    if (currentState === states.start) {
      if (blocksUsed < numOfBlocks) {
        ...
        for (let i = 0; i < allMaterials.length; i++) {
          if (allMaterials[i].name === move + "_block_mat") {
            block.material = allMaterials[i];
          }
        }
  ...
  function setTexture(texture_name, mat_name) {
    for (let i = 0; i < allTextures.length; i++) {
      if (allTextures[i].name === texture_name) {
        let signal = allTextures[i].signal;
        for (let j = 0; j < allMaterials.length; j++) {
          if (allMaterials[j].name === mat_name) {
            allMaterials[j].setTextureSlot("DIFFUSE", signal);
          }
        }
      }
    }
  ...
  function setLoopIterations(i) {
    ...
    if (
      ...
      loopIterations = i;
      setTexture("loop_" + i + "_block", "loop_2_block_mat");
    }
  }
  ...
  function changeState(state, buttonText) {
    Time.setTimeout(function () {
      currentState = state;
      setTexture(buttonText, "btn_play_mat");
    }, 500);
  }
})();
